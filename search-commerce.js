...
async function main() {
  console.log('=== 搜索电商爬虫 ===');
  const result = await searchRepo('电商+爬虫+OR+e-commerce+spider+OR+shopify+spider');
  ...
  console.log('=== 搜索商品爬虫 ===');
  const result2 = await searchRepo('商品+爬虫+OR+product+spider+OR+goods+spider');
  ...
  console.log('=== 搜索价格监控 ===');
  const result3 = await searchRepo('价格监控+OR+price+monitor+OR+price+tracker');
